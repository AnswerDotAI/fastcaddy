...
fastcaddy talks to caddy’s admin API, and config set that way is
*ephemeral*: it doesn’t survive a caddy restart. So the recommended
pattern is a setup script that runs on every deploy (e.g. from systemd):
call
[`reset()`](https://AnswerDotAI.github.io/fastcaddy/core.html#reset),
then the functions below for everything you host. Each function is safe
to re-run – routes are tagged with predictable `@id`s (the hostname, or
`wildcard-{domain}`), and re-adding one replaces it rather than
duplicating it – so the script is the single source of truth for your
routing config.
...
Caddy can create a wildcard SSL cert. To do so, add a wildcard route (a
no-op if it already exists, so existing subdomains inside it are never
destroyed):
