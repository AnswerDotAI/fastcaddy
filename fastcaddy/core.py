...
# %% auto #0
__all__ = ['admin_url', 'automation_path', 'srvs_path', 'rts_path', 'get_id', 'get_path', 'gid', 'has_id', 'gcfg', 'has_path',
           'pid', 'pcfg', 'reset', 'nested_setdict', 'path2keys', 'keys2path', 'nested_setcfg', 'init_path',
           'get_acme_config', 'add_tls_internal_config', 'add_acme_config', 'caddy_docs', 'get_schema', 'search_schema',
           'init_routes', 'setup_pki_trust', 'setup_caddy', 'encode_handler', 'proxy_handler', 'add_route', 'del_id',
           'add_reverse_proxy', 'add_on_demand_tls', 'add_wildcard_route', 'add_sub_route', 'add_sub_reverse_proxy']
...
# %% ../nbs/00_core.ipynb #b4b503f6
def get_id(path):
    "Get an ID full URL from a path"
    if path[0]!='/': path = '/'+path
    if path[-1]!='/': path = path+'/'
    return f'{admin_url}/id{path}'

# %% ../nbs/00_core.ipynb #baad1fbf
def get_path(path):
    "Get a config full URL from a path"
    if path[0]!='/': path = '/'+path
    if path[-1]!='/': path = path+'/'
    return f'{admin_url}/config{path}'

# %% ../nbs/00_core.ipynb #9c89064b
def _req(f, url, **kw):
    "Request `url` via `f`, raising errors enriched with caddy's detail; returns the response"
    try: response = f(url, **kw)
    except ConnectError as e:
        e.add_note(f"Could not reach the caddy admin API at {admin_url} -- is caddy running?")
        raise
...
# %% ../nbs/00_core.ipynb #a991407f
def gid(path='/'):
    "Get the config object whose `@id` matches `path`"
    return dict2obj(_req(xget, get_id(path)).json())

# %% ../nbs/00_core.ipynb #ac40f2d8
def has_id(id):
...
# %% ../nbs/00_core.ipynb #7ca93d12
def gcfg(path='/'):
    "Get the config at `path`"
    return dict2obj(_req(xget, get_path(path)).json())

# %% ../nbs/00_core.ipynb #a4a2a609
def has_path(path):
    "Check if any config exists at `path`"
    try: return gcfg(path) is not None
    except HTTPStatusError: return False

# %% ../nbs/00_core.ipynb #c25a2e11
def pid(d, path='/', method='post'):
    "Put/post config `d` to the object whose `@id` matches `path`"
    f = getattr(httpx, method)
    return _req(f, get_id(path), json=obj2dict(d)).text or None

# %% ../nbs/00_core.ipynb #41e5f6ee
def pcfg(d, path='/', method='post'):
    "Put/post config `d` at `path`"
    f = getattr(httpx, method)
    return _req(f, get_path(path), json=obj2dict(d)).text or None

# %% ../nbs/00_core.ipynb #421d4ed3
def reset():
...
def del_id(id):
    ...
    while has_id(id): _req(xdelete, get_id(id))
